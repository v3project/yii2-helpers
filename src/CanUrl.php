...
class CanUrl extends Component implements BootstrapInterface {
    ...
    public function SETquery_params($query_params) {
        if (isset($query_params) AND empty($query_params)) $query_params = [];
        if (isset($query_params) AND !is_array($query_params)) throw new InvalidParamException('(isset($query_params) AND !is_array($query_params))');
        $this->_query_params = $query_params;
        return $this;
    }
    public function GETquery_params() {
        if (isset($this->_query_params) AND !is_array($this->_query_params)) throw new Exception('(isset($this->_query_params) AND !is_array($this->_query_params))');
        return $this->_query_params;
    }
    public function ADDquery_params($add_query_params) {
        if (empty($add_query_params)) $add_query_params = [];
        if (!is_array($add_query_params)) throw new InvalidParamException('(!is_array($add_query_params))');
    ...
    public function GETquery() {
        if (!isset($this->_query_params)) return null;
    ...
    public function GETcanurl() {
        ...
        if (!isset($this->_scheme)) throw new UserException('!isset($this->_scheme)');
        if (!isset($this->_user)) throw new UserException('!isset($this->_user)');
        if (!isset($this->_pass)) throw new UserException('!isset($this->_pass)');
        if (!isset($this->_host)) throw new UserException('!isset($this->_host)');
        if (!isset($this->_port)) throw new UserException('!isset($this->_port)');
        if (!isset($this->_path)) throw new UserException('!isset($this->_path)');
        if (!isset($this->_fragment)) throw new UserException('!isset($this->_fragment)');
    ...
    public function SETimportant_params($important_params) {
        if (isset($important_params) AND empty($important_params)) $important_params = [];
        if (isset($important_params) AND !is_array($important_params)) throw new InvalidParamException('(isset($important_params) AND !is_array($important_params))');
        $this->_important_params = $important_params;
        return $this;
    }
    public function GETimportant_params() {
        if (isset($this->_important_params) AND !is_array($this->_important_params)) throw new Exception('(isset($this->_important_params) AND !is_array($this->_important_params))');
    ...
    public function ADDimportant_params($add_important_params) {
        ...
        if (!is_array($add_important_params)) throw new InvalidParamException('(!is_array($add_important_params))');
    ...
    public function SETimportant_pnames($important_pnames) {
        ...
        if (!is_array($important_pnames)) throw new InvalidParamException('(!is_array($important_pnames))');
    ...
    public function ADDimportant_pnames($add_important_pnames) {
        ...
        if (!is_array($add_important_pnames)) throw new InvalidParamException('(!is_array($add_important_pnames))');
    ...
    public function SETminor_params($minor_params) {
        if (isset($minor_params) AND empty($minor_params)) $minor_params = [];
        if (isset($minor_params) AND !is_array($minor_params)) throw new InvalidParamException('(isset($minor_params) AND !is_array($minor_params))');
        $this->_minor_params = $minor_params;
        return $this;
    }
    public function GETminor_params() {
        if (isset($this->_minor_params) AND !is_array($this->_minor_params)) throw new Exception('(isset($this->_minor_params) AND !is_array($this->_minor_params))');
        return $this->_minor_params;
    }
    public function ADDminor_params($add_minor_params) {
        if (empty($add_minor_params)) $add_minor_params = [];
        if (!is_array($add_minor_params)) throw new InvalidParamException('(!is_array($add_minor_params))');
    ...
    public function SETminor_pnames($minor_pnames) {
        ...
        if (!is_array($minor_pnames)) throw new InvalidParamException('(!is_array($minor_pnames))');
    ...
    public function ADDminor_pnames($add_minor_pnames) {
        ...
        if (!is_array($add_minor_pnames)) throw new InvalidParamException('(!is_array($add_minor_pnames))');
    ...
    public function is_need_redirect($is_final, $current_url = null) {
        if (!is_bool($is_final)) throw new InvalidParamException('(!is_bool($throw_if_null))');
        if (!isset($current_url)) $current_url = \Yii::$app->getRequest()->getAbsoluteUrl();
        if (!is_string($current_url)) throw new InvalidParamException('(!is_string($current_url))');
    ...
    public function GETredirurl($current_url = null, $is_final = true) {
        if (!isset($current_url)) $current_url = \Yii::$app->getRequest()->getAbsoluteUrl();
        if (!is_string($current_url) AND !is_array($current_url)) throw new InvalidParamException('(!is_string($current_url) AND !is_array($current_url))');
        ...
        if (!is_bool($is_final)) throw new InvalidParamException('(!is_bool($is_final))');
        ...
        if ($is_final) {
            if (!isset($this->_scheme)) throw new UserException('!isset($this->scheme)');
            if (!isset($this->_user)) throw new UserException('!isset($this->user)');
            if (!isset($this->_pass)) throw new UserException('!isset($this->pass)');
            if (!isset($this->_host)) throw new UserException('!isset($this->host)');
            if (!isset($this->_port)) throw new UserException('!isset($this->port)');
            if (!isset($this->_path)) throw new UserException('!isset($this->path)');
            if (!isset($this->_fragment)) throw new UserException('!isset($this->fragment)');

            if (!isset($this->_query_params)) throw new UserException('!isset($this->query_params)');
        }
    ...
    public function make_query_for_redirurl($current_url = null, $is_final = true) {
        ...
        $current_params = [];
        if (empty($parsed_current_url['query'])) $current_params = [];
        else parse_str($parsed_current_url['query'], $current_params);


        if (!isset($this->_query_params)) return null;
        if (!isset($this->_important_params) AND !$is_final) return null;
        if (!isset($this->_minor_params) AND !$is_final) return null;
        ...
        foreach ($this_important_params as $kkk => $vvv) {
            if (!isset($vvv) AND array_key_exists($kkk, $current_params)) $vvv = $current_params[$kkk];
            $rett_params[$kkk] = $vvv;
        }
